	 ...
	 */
     function goStartMode(){
    	 ...
    	 if(sSrc.indexOf("btn_confirm2.png") < 0 ){
    		 welBtnConfirm.attr("src","/editor/img/photoQuickPopup/btn_confirm2.png");
     ...
     function goReadyMode(){
    	 ...
    	 if(sSrc.indexOf("btn_confirm2.png") >= 0 ){
	    	 ...
	    	 welBtnConfirm.attr("src","/editor/img/photoQuickPopup/btn_confirm.png");
    	 }
     }   
    ...
    function addImage(ofile){
		...
		if( !!bExceedLimitTotalSize ){
	        ...
	        aFileList.push('	<a onclick="delImage(\'img'+nImageInfoCnt+'\')"><img class="del_button" src="/WEB-INF/resource/editor/img/photoQuickPopup/btn_del.png"  width="14" height="13" alt="첨부 사진 삭제"></a>');
			aFileList.push('	</li> ');
...
//    	sUploadURL= 'file_uploader_html5.php'; 	//upload URL
    	sUploadURL= "/common/file/fileUpload"; 	//upload URL
...
// 	    		oCustomEvent.stop(); 수행시 bAllowed 가 false이더라도 alert이 수행되지 않음
 	    	...
 	    	error : function(oCustomEvent) {
 	    		...
 	    		alert(errorMsg(oCustomEvent.htResult.errstr));
 	    	}
 	    });
 	}
     ...
     */
 	function errorMsg(code){
 		var msg;
 		if(code == "0"){
 			msg = "업로드 할 수 없는 파일형식입니다.";
 		}else if(code == "1"){
 			msg = "업로드 할 수 없는 파일명입니다.";
 		}else if(code == "2"){
 			msg = "용량이 1MB 이상이거나 해상도가 4000*4000 이상인 이미지는 첨부가 불가능합니다.";
 		}else if(code == "3"){
 			msg = "용량이 1MB 이상이거나 해상도가 4000*4000 이상인 이미지는 첨부가 불가능합니다.";
 		}else if(code == "4"){
 			msg = "파일이 없습니다.";

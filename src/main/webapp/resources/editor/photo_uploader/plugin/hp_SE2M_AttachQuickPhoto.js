...
nhn.husky.SE2M_AttachQuickPhoto = jindo.$Class({		
	...
	makePopupURL : function(){
		var sPopupUrl = "/common/file/formFile";
	...
	$ON_SET_PHOTO : function(aPhotoData){
		...
		try{
			...
			for(var i = 0; i <aPhotoData.length; i++){				
				...
				var imgWidth = htData.sFileWidth;
				var imgHeight = htData.sFileHeight;

				/*if( htData.sFileWidth > 200 ){
					imgWidth = 200;
				}else{
					imgWidth = htData.sFileWidth
				}

				if( htData.sFileHeight > 300 ){
					imgHeight = 300;
				}else{
					imgHeight = htData.sFileHeight
				}*/
	...
	_getPhotoTag : function(htPhotoInfo){
		...
		var sTag = '<img src="{=sOriginalImageURL}" title="{=sName}" style="width:{=sFileWidth}px;" >';
